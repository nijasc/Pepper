...
public final class AnimationGenerator {
    ...
    private String danceCompactPrompt() {
        return "You are a choreographer for the Pepper robot. Output ONLY a compact JSON object describing "
                ...
                + "the return.\n"
                + "- Make it a REAL, artistic choreography, not a wiggle: every keyframe is a distinct, "
                + "deliberate pose that flows GENTLY and smoothly into the next, like calm, graceful dancing. "
                + "Favor soft, sweeping arm arcs (slow reaches, gentle waves, wide circles, openings and "
                + "closings), gentle swaying of the head and upper body, and let the motif EVOLVE - vary the "
                + "poses across the cycle instead of repeating a single beat. Contrast the left and right arms. "
                + "Use the full arm range so the moves read clearly from across a room, but move calmly and "
                + "fluidly rather than fast or sharply. Keep the lower body gentle for balance with small smooth "
                + "steps.\n"
